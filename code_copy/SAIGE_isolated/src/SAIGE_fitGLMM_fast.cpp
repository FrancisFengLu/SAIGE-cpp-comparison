...
struct indicesRelatedSamples : public RcppParallel::Worker {
  ...
  indicesRelatedSamples(int Ntotal, std::vector< std::pair<int, int> > &output, std::mutex &output_mutex) :
    Ntotal(Ntotal), output(output), output_mutex(output_mutex) {}
...
//  indicesRelatedSamples indicesRelatedSamples(Ntotal,output);
  indicesRelatedSamples indicesRelatedSamples(Ntotal,geno.indiceVec,geno.indiceVec_mutex);
...
float GetTrace(arma::fmat Sigma_iX, arma::fmat& Xmat, arma::fvec& wVec, arma::fvec& tauVec, arma::fmat& cov1, int nrun, int maxiterPCG, float tolPCG, float traceCVcutoff){
  ...
  if (outfile.is_open()) {
    cout << "=== SAVING RANDOM VECTORS FOR C++ BYPASS ===" << endl;
...
int nrun, int maxiterPCG, float tolPCG, float traceCVcutoff){
	...
	//PAPY_1.print("PAPY_1");
  	...
  	return Rcpp::List::create(Named("YPAPY") = YPAPY, Named("Trace") = Trace, Named("PY") = PY1, Named("APY") = APY, Named("AI") = AI);
}
...
int nrun, int maxiterPCG, float tolPCG, float tol, float traceCVcutoff){
	...
	//std::cout << "fitglmmaiRPCG starts 2" << std::endl;
  	...
  	tauVec(1) = tau0(1) + Dtau;
        // ============ DEBUG OUTPUT for comparison with C++ standalone ============
        std::cout << "\n=== R fitglmmaiRPCG DEBUG ===" << std::endl;
        std::cout << "tau_before: [" << tau0(0) << ", " << tau0(1) << "]" << std::endl;
        std::cout << "YPAPY: " << YPAPY << std::endl;
        std::cout << "Trace: " << Trace << std::endl;
        std::cout << "Score (YPAPY-Trace): " << score1 << std::endl;
        std::cout << "AI: " << AI1 << std::endl;
        std::cout << "Dtau (Score/AI): " << Dtau << std::endl;
        std::cout << "tau_after (before clamping): [" << tauVec(0) << ", " << tauVec(1) << "]" << std::endl;
        std::cout << "==============================\n" << std::endl;
  	...
  	return List::create(Named("tau") = tauVec, Named("YPAPY") = YPAPY, Named("Trace") = Trace, Named("AI") = AI1, Named("Score") = score1);
}
