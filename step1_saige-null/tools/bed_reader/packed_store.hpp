...
class PackedFlat {
 public:
  ...
  void init(std::size_t capacity_markers, std::size_t nbyte_per_marker) {
    ...
    data_.assign(capacity_markers * nbyte_per_marker, 0);
    n_stored_ = 0;
  ...
  std::size_t append(const unsigned char* bytes) {
    ...
    std::memcpy(data_.data() + n_stored_ * nbyte_, bytes, nbyte_);
    return n_stored_++;
  }
  ...
  void write(std::size_t compact_idx, const unsigned char* bytes) {
    ...
    std::memcpy(data_.data() + compact_idx * nbyte_, bytes, nbyte_);
  }
  ...
  PackedSpan span(std::size_t compact_idx) const {
    ...
    return { data_.data() + compact_idx * nbyte_, nbyte_ };
  }
  ...
  const unsigned char* raw() const { return data_.data(); }
