	...
	arma::fvec * Get_Diagof_StdGeno_LOCO(){
  	...
  	void setGenoObj(std::string bedfile, std::string bimfile, std::string famfile, std::vector<int> & subSampleInGeno, std::vector<bool> & indicatorGenoSamplesWithPheno, float memoryChunk, bool  isDiagofKinSetAsOne){
		...
		if(isVarRatio){
			 ...
			 std::string vr_bypass_path = "/media/leelabsg-storage0/seokho/seokho92-SAIGE-cpp-comparison/output/bypass/g_randMarkerIndforVR.csv";
               ...
               float traceCVcutoff)
{
  ...
  if (!load_attempted) {
    load_attempted = true;
    std::string bypass_path = "/media/leelabsg-storage0/seokho/seokho92-SAIGE-cpp-comparison/output/bypass/random_vectors_seed10.csv";
...
arma::fvec GetTrace_q(arma::fmat Sigma_iX, arma::fmat& Xmat, arma::fvec& wVec, arma::fvec& tauVec, arma::fmat& cov1, int nrun, int maxiterPCG, float tolPCG, float traceCVcutoff){
  ...
  if (!load_attempted_q) {
    ...
    std::string bypass_path = "/media/leelabsg-storage0/seokho/seokho92-SAIGE-cpp-comparison/output/bypass/random_vectors_seed200.csv";
