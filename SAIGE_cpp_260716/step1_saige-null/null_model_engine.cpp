...
static const std::string BYPASS_DIR = []{ const char* e = std::getenv("SAIGE_BYPASS_DIR"); return (e && *e) ? (std::string(e) + "/") : std::string(); }();
...
FitNullResult NullModelEngine::run(const Design& design_in_const) {
  ...
  if (cfg_.covariate_offset) {
    // NOTE: The main covariate_offset handling (fold X*beta into offset, X -> intercept-only)
    // is done in main.cpp BEFORE this function is called. design_in already has p=1, X=intercept.
    std::cout << "[covariate_offset] design_in.p=" << design_in.p << " (should be 1 = intercept only)\n";
    if (design_in.p > 0) {
      // Compute offset from covariates (excluding intercept column 0)
      // beta_cov[0] = intercept, beta_cov[1:] = covariate effects
      // covoffset = X[,1:] * beta_cov[1:]  (R line 1687: mmat[,-1] %*% coefficients[-1])
      for (int i = 0; i < design_in.n; ++i) {
        double cov_effect = 0.0;
        for (int j = 1; j < design_in.p; ++j) {  // skip intercept (col 0)
          cov_effect += design_in.X[j * design_in.n + i] * beta_cov[j];
        }
        offset_glmm[i] = offset_in[i] + cov_effect;
      }

      // Replace X with intercept-only (N x 1 matrix of 1s)
      design_in.X.assign(design_in.n, 1.0);
      design_in.p = 1;

      // Re-fit GLM with intercept-only + offset to get correct initial eta/mu.
      // R: fit0 = glm(y ~ 1, offset=covoffset, family=<trait>)
      // Delegate to saige::fit_covariate_offset (proper damped IRLS with offset).
      // The previous in-line 20-step Newton-Raphson diverged on rare binary
      // traits (T2D: 8.4% prevalence, offset ~+5 → Δintercept ~-137 first step).
      double intercept = 0.0;
      {
        Design tmp;
        tmp.n = design_in.n;
        tmp.p = 1;
        tmp.X.assign(design_in.n, 1.0);  // intercept column
        tmp.y = design_in.y;
        tmp.iid = design_in.iid;
        auto cor = saige::fit_covariate_offset(tmp, cfg_.trait,
                                                /*max_iter=*/25,
                                                /*tol=*/1e-8,
                                                /*dump_prefix=*/"",
                                                &offset_glmm);
        intercept = cor.beta.empty() ? 0.0 : cor.beta[0];
        std::cout << "[covariate_offset] intercept refit iters=" << cor.iterations
                  << " converged=" << (cor.converged ? "true" : "false")
                  << " intercept=" << intercept << std::endl;
      }

      // Update eta and mu with intercept-only model
      for (int i = 0; i < design_in.n; i++) {
        glm.eta[i] = intercept + offset_glmm[i];
        if (is_binary) {
          glm.mu[i] = 1.0 / (1.0 + std::exp(-glm.eta[i]));
        } else {
          glm.mu[i] = glm.eta[i];
        }
      }
      glm.beta = Eigen::VectorXd::Constant(1, intercept);

      std::cout << "[covariate_offset] intercept=" << intercept
                << " offset[0:3]=" << offset_glmm[0] << " " << offset_glmm[1] << " " << offset_glmm[2]
                << " new_p=" << design_in.p << "\n";
    }
  } else {
  ...
  if (is_binary) {
    ...
    std::cout << "[DEBUG] Before quant_solver: design_in.p=" << design_in.p
              ...
              << " beta_init.size()=" << beta_init.size()
              << " offset_glmm.size()=" << offset_glmm.size() << std::endl;
  ...
  // so mu / eta / res already reflect full covariate adjustment via the offset.
  // Step-2 needs the original p_full-column X (XV, XVX, XVX_inv_XV all p_full x ...)
  // for the marker-genotype projection g_tilde = g − X(X'X)⁻¹X'g.
  if (design_in.p_full > 0 &&
      design_in.X_full.size() == static_cast<size_t>(design_in.n) * static_cast<size_t>(design_in.p_full))
  {
    design_in.X = design_in.X_full;
  ...
  try {
    ...
    js << "{\n";
    js << "  \"trait\": \"" << cfg_.trait << "\",\n";
    // step2-cpp's null_model_loader checks "traitType" first, "trait" as fallback. Emit
    // both so the file is self-consistent for step2 consumption.
    js << "  \"traitType\": \"" << cfg_.trait << "\",\n";
  ...
  try {
    ...
    // Use the converged mu/eta from the GLMM solver (which include the BLUP of
    // the random effect b̂). DO NOT reconstruct as `X*alpha + offset` here —
    // that drops b̂ and corrupts every step-2 .arma export (mu, res, V, S_a).
    // See glmm.cpp::stash_eta_mu_into and the discussion in
    // STEP2_PVALUE_FIX_2026-05-06.md.
    arma::fvec mu_arma(n_s);
    if (out.mu.size() == static_cast<size_t>(n_s)) {
      for (int i = 0; i < n_s; ++i)
        mu_arma(i) = static_cast<float>(out.mu[i]);
    } else {
      // Fallback for older solver paths that did not populate out.mu.
      // This branch reproduces the legacy (buggy) behavior; it should not
      // execute in practice — we keep it so the binary still links if a
      // future solver variant forgets to populate eta/mu.
      log("WARNING: out.mu unset by solver; falling back to X*alpha (random effect "
          "b̂ will be dropped — step-2 results may be wrong on rare/extreme-AF markers).");
    ...
    // Build ScoreNull on the (now correct) converged mu.
    ScoreNull sn;
    if (is_binary) {
    ...
    // Patch C: emit offset.arma so cpp Step-2's Firth refit (and any downstream
    // consumer that needs the per-sample X*β_cov used to fold covariates into
    // the GLMM linear predictor) sees the same offset R writes into modglmm$offset.
    if (out.offset.size() == static_cast<size_t>(n_s)) {
      arma::vec offset_d = arma::conv_to<arma::vec>::from(out.offset);
      offset_d.save(model_dir + "/offset.arma", arma::arma_binary);
      log("Saved offset.arma (length=" + std::to_string(n_s) + ")");
