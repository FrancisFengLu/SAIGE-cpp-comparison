...
// Checkpoint/debug CSV dump directory. Opt-in only: set the SAIGE_CP_DIR
// environment variable to enable; unset (default) => checkpoints disabled
// (no files written). Retained for R-comparison debugging.
static const std::string CP_DIR = []{
  const char* e = std::getenv("SAIGE_CP_DIR");
  return (e && *e) ? std::string(e) : std::string();
}();
static const bool CP_ON = !CP_DIR.empty();
static const bool CP_INIT = []{
  if (CP_ON) { std::error_code ec; std::filesystem::create_directories(CP_DIR, ec); }
  return true;
}();
// Resolve a checkpoint file path; empty when disabled so the writers no-op.
static inline std::string cpf(const std::string& name) {
  return CP_ON ? (CP_DIR + "/" + name) : std::string();
}
...
// Stash the converged linear predictor and fitted values onto FitNullResult.
// MUST be called from each return path of binary_glmm_solver / quant_glmm_solver
// so downstream consumers (.arma export in null_model_engine.cpp, VR computation
// in variance_ratio_compute.cpp) do not re-derive eta as X*alpha and drop the
// BLUP of the random effect.
static inline void stash_eta_mu_into(FitNullResult& out,
                                     const arma::fvec& eta_final,
                                     const arma::fvec& mu_final)
{
  out.eta.resize(eta_final.n_elem);
  out.mu.resize(mu_final.n_elem);
  for (arma::uword i = 0; i < eta_final.n_elem; ++i)
    out.eta[i] = static_cast<double>(eta_final(i));
  for (arma::uword i = 0; i < mu_final.n_elem; ++i)
    out.mu[i]  = static_cast<double>(mu_final(i));
}
...
check_dims("inputs");
  ...
  // Save to files
  {
    std::ofstream f(cpf("CPP_CP2_initial.txt"));
    f << "tau: " << tau[0] << " " << tau[1] << "\n";
    f << "n: " << n << "\n";
    f << "p: " << p << "\n";
    f << "beta_init: ";
  ...
  save_vec_csv(eta, cpf("CPP_CP2_eta.csv"));
  save_vec_csv(y, cpf("CPP_CP2_y.csv"));
  if (CP_ON) std::cout << "CP2 saved to: " << CP_DIR << "/CPP_CP2_*.csv/txt" << std::endl;
  // ============ END CHECKPOINT 2 ============
...
check_coef("getCoefficients_cpp");
    ...
    if (it == 0) {
      ...
      // Save to files
      {
        std::ofstream f(cpf("CPP_CP3_coef.txt"));
        f << "alpha: ";
      ...
      save_vec_csv(coef.Sigma_iY, cpf("CPP_CP3_Sigma_iY.csv"));
      save_mat_csv(coef.Sigma_iX, cpf("CPP_CP3_Sigma_iX.csv"));
      save_vec_csv(coef.eta, cpf("CPP_CP3_eta.csv"));
      save_vec_csv(Y, cpf("CPP_CP3_Y.csv"));
      save_vec_csv(W, cpf("CPP_CP3_W.csv"));
      if (CP_ON) std::cout << "CP3 saved to: " << CP_DIR << "/CPP_CP3_*.csv/txt" << std::endl;
    }
    // ============ END CHECKPOINT 3 ============
...
if ((int)W.n_elem != n) throw std::runtime_error("W.n_elem!=n before AI");
    ...
    if (it == 0) {
      ...
      // Save to files
      {
        std::ofstream f(cpf("CPP_CP4_AIScore.txt"));
        f << std::setprecision(10);
        f << "YPAPY: " << ai.YPAPY << "\n";
        f << "Trace: " << ai.Trace << "\n";
        f << "AI: " << ai.AI << "\n";
      ...
      save_vec_csv(ai.PY, cpf("CPP_CP4_PY.csv"));
      save_vec_csv(ai.APY, cpf("CPP_CP4_APY.csv"));
      if (CP_ON) std::cout << "CP4 saved to: " << CP_DIR << "/CPP_CP4_*.csv/txt" << std::endl;
    }
    // ============ END CHECKPOINT 4 ============
    ...
    if (it == 2) {
      ...
      {
        std::ofstream f(cpf("CPP_CP4b_iter2.txt"));
      ...
      save_vec_csv(W, cpf("CPP_CP4b_W.csv"));
      save_vec_csv(Y, cpf("CPP_CP4b_Y.csv"));
      save_vec_csv(mu, cpf("CPP_CP4b_mu.csv"));
      save_vec_csv(eta, cpf("CPP_CP4b_eta.csv"));
      save_vec_csv(coef.Sigma_iY, cpf("CPP_CP4b_Sigma_iY.csv"));
      save_vec_csv(ai.PY, cpf("CPP_CP4b_PY.csv"));
      save_vec_csv(ai.APY, cpf("CPP_CP4b_APY.csv"));
      if (CP_ON) std::cout << "CP4b saved to: " << CP_DIR << "/CPP_CP4b_*.csv/txt" << std::endl;
    }
    // ============ END CHECKPOINT 4b ============
    ...
    // R: if(tau[2] == 0) break  — exact zero check, not <=0
    // NOTE: Skip on conservative iter 0 (tau starts [1,0], step = tau[1]^2·score/n = 0
    // keeps tau[1] at 0 by construction). R's break applies only to main-loop iters.
    if (it > 0 && tau[1] == 0.0f) {
      // R (line 639): if(tau[2] == 0) break. The genetic variance at the 0
      // boundary is a CONVERGED boundary solution in R (converged=TRUE with
      ...
      out.offset = offset_in;
      stash_score_null_into(out, sn, n, p);
      stash_eta_mu_into(out, eta, mu_final);
      out.converged = true;
      out.iterations = it + 1;
      export_score_null_json(paths, out);

      std::cout << "=== CONVERGED (tau[1] at 0 boundary) at iteration " << it
                << " ===" << std::endl;
      std::cout << "Final tau: [" << tau[0] << ", " << tau[1] << "]" << std::endl;
      std::cout << "Converged: true" << std::endl;
      std::cout << "Iterations: " << (it + 1) << std::endl;
      {
        std::ofstream f_cp6(cpf("CPP_CP6_final.csv"));
        f_cp6 << "tau0," << tau[0] << "\ntau1," << tau[1] << "\n";
        f_cp6 << "converged,1\niterations," << (it + 1) << "\n";
        for (size_t i = 0; i < alpha.n_elem; ++i)
    ...
    {
      std::ofstream f(cpf("CPP_iteration_summary.csv"),
    ...
    if (STOP_AFTER_FIRST_TAU_UPDATE && std::fabs(Dtau) > 1e-10) {
      ...
      if (real_tau_updates >= 4) {
        ...
        std::ofstream f(cpf("CPP_FIRST_TAU_UPDATE.txt"));
        f << std::setprecision(12);
        f << "iteration: " << it << "\n";
        ...
        save_vec_csv(eta, cpf("CPP_FIRST_TAU_eta.csv"));
        save_vec_csv(W, cpf("CPP_FIRST_TAU_W.csv"));
        save_vec_csv(Y, cpf("CPP_FIRST_TAU_Y.csv"));
        save_vec_csv(ai.PY, cpf("CPP_FIRST_TAU_PY.csv"));
        save_vec_csv(ai.APY, cpf("CPP_FIRST_TAU_APY.csv"));
        save_vec_csv(coef.Sigma_iY, cpf("CPP_FIRST_TAU_Sigma_iY.csv"));
    ...
    if (it == 0) {
      ...
      // Save to files
      std::ofstream f_cp5(cpf("CPP_CP5_iteration1.csv"));
      f_cp5 << "tau0," << tau[0] << "\n";
      f_cp5 << "tau1," << tau[1] << "\n";
      f_cp5 << "Score," << score << "\n";
      f_cp5 << "Dtau," << Dtau << "\n";
      ...
      f_cp5 << "AI," << AI << "\n";
      f_cp5.close();
      if (CP_ON) std::cout << "Saved: " << CP_DIR << "/CPP_CP5_iteration1.csv" << std::endl;
    }
    ...
    if (it > 0 && rc_tau < tol_coef) {
      ...
      f_cp6.close();
      if (CP_ON) std::cout << "Saved: " << CP_DIR << "/CPP_CP6_final.csv" << std::endl;
                                ...
                                const std::vector<double>& beta_init_in)
{
  ...
  for (int it = 0; it < maxiter; ++it) {
    ...
    // ===== Dump inputs to getAIScore_q at iteration 1 for R comparison =====
    if (it == 1) {
      std::cout << "\n===== INPUTS TO getAIScore_q (iter 1) =====" << std::endl;
      std::cout << "Y[0:5]: " << Y[0] << " " << Y[1] << " " << Y[2] << " " << Y[3] << " " << Y[4] << std::endl;
      std::cout << "|Y|=" << arma::norm(Y) << " mean(Y)=" << arma::mean(Y) << " var(Y)=" << arma::var(Y) << std::endl;
      std::cout << "W[0:5]: " << W[0] << " " << W[1] << " " << W[2] << " " << W[3] << " " << W[4] << std::endl;
      std::cout << "tau: [" << tau[0] << ", " << tau[1] << "]" << std::endl;
      std::cout << "X dim: " << X.n_rows << "x" << X.n_cols << std::endl;
      std::cout << "X[0:3,0]: " << X(0,0) << " " << X(1,0) << " " << X(2,0) << std::endl;
      std::cout << "Sigma_iY[0:5]: " << coef.Sigma_iY[0] << " " << coef.Sigma_iY[1] << " " << coef.Sigma_iY[2] << " " << coef.Sigma_iY[3] << " " << coef.Sigma_iY[4] << std::endl;
      std::cout << "|Sigma_iY|=" << arma::norm(coef.Sigma_iY) << std::endl;
      std::cout << "Sigma_iX[0:3,0]: " << coef.Sigma_iX(0,0) << " " << coef.Sigma_iX(1,0) << " " << coef.Sigma_iX(2,0) << std::endl;
      std::cout << "cov diag: " << coef.cov(0,0) << std::endl;
      std::cout << "alpha: [";
      for (size_t i = 0; i < coef.alpha.n_elem; ++i) {
        std::cout << coef.alpha[i]; if (i < coef.alpha.n_elem-1) std::cout << ", ";
      }
      std::cout << "]" << std::endl;
      std::cout << "eta[0:5]: " << eta[0] << " " << eta[1] << " " << eta[2] << " " << eta[3] << " " << eta[4] << std::endl;
      std::cout << "offset[0:5]: " << offset[0] << " " << offset[1] << " " << offset[2] << " " << offset[3] << " " << offset[4] << std::endl;
    ...
    if (it == 0) {
      ...
      // GMMAT "zero-the-boundary-component" trick, ported verbatim from R's
      // fitglmmaiRPCG_q (SAIGE_isolated/src/SAIGE_fitGLMM_fast.cpp:6210-6254).
      // A component that was already ~0 (< tol) BEFORE the update and lands < tol
      // AFTER is pinned to exactly 0 — so the `tau_new < 0` step-halving check sees
      // 0 (not a negative), and does NOT freeze the joint step. Without this, when
      // tau[1] starts at 0 and delta[1] < 0, halving drives step->0 and freezes
      // tau[0] too (the bug that left LDL sparse-fit stuck at tau[1]=0).
      arma::uvec zeroVec(2);
      zeroVec[0] = (tau[0] < tol_coef) ? 1u : 0u;
      zeroVec[1] = (tau[1] < tol_coef) ? 1u : 0u;
      auto pin_boundary = [&](arma::fvec& t) {
        for (int j = 0; j < 2; ++j)
          if (zeroVec[j] && t[j] < tol_coef) t[j] = 0.0f;
      };

      tau_new = tau + delta;
      pin_boundary(tau_new);

      // Step-halving ONLY on a genuine negative (boundary comps already pinned to 0).
      double step = 1.0;
      ...
      // Final: any sub-tol component -> exactly 0 (R: tauVec.elem(find(tauVec<tol)).zeros()).
      if (tau_new[0] < tol_coef) tau_new[0] = 0.0f;
      if (tau_new[1] < tol_coef) tau_new[1] = 0.0f;

      std::cout << "[STANDARD AI-REML] delta: [" << delta[0] << ", " << delta[1] << "]"
                << "  zeroVec=[" << zeroVec[0] << "," << zeroVec[1] << "]  step: " << step << std::endl;
    }
    ...
    // NOTE: In R, this check only applies to main-loop iterations (i>=1),
    // NOT to the pre-loop conservative step (it==0). The conservative step
    // uses tau^2 * score / n, which always gives 0 when tau starts at 0.
    // R continues into the main loop where AI-REML can produce non-zero tau.
    if (it > 0 && (tau[0] <= 0.0f || tau[1] <= 0.0f)) {
      // R (line 941): if(tau[1]<=0 | tau[2]<=0) break. R treats the genetic
      // variance hitting the 0 boundary as a CONVERGED boundary solution
      // (converged=TRUE with tau[1]=0), NOT a failure. Match R: clamp the
      // genetic VC to 0 and finalize as converged when the residual variance
      // tau[0] is still positive. A non-positive tau[0] is genuinely
      // degenerate -> keep the old not-converged break.
      const bool boundary_ok = (tau[0] > 0.0f);
      if (tau[1] < 0.0f) tau[1] = 0.0f;
      std::cout << "[quant_glmm] tau=[" << tau[0] << ", " << tau[1]
                << "] at boundary; "
                << (boundary_ok ? "clamped tau[1]=0, finalizing as converged (matches R)."
      ...
      if (boundary_ok) {
        ...
        std::cout << "Iterations: " << (it + 1) << std::endl;
        {
          std::ofstream f(cpf("CPP_CP6_final.csv"));
          f << "tau0," << tau[0] << "\ntau1," << tau[1] << "\n";
          f << "converged,1\niterations," << (it + 1) << "\n";
          for (size_t i = 0; i < coef.alpha.n_elem; ++i)
