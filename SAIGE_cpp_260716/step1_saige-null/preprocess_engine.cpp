...
PreOut PreprocessEngine::run(const Design& design_in) {
  ...
  if (!cfg_.use_sparse_grm_to_fit && !paths_.fam.empty()) {
    ...
    // Sparse-fit (or neither FAM nor sparse IDs provided): keep the design's
    // CURRENT order. main.cpp has already reordered the design to ascending
    // FAM-row order *and* subset the sparse GRM K in that same order. Reordering
    // here to the sparse-GRM sampleID-file order (read_sparse_iids_) would leave
    // the working vectors (y/X/offset -> PY) in GRM-file order while K stays in
    // FAM order, scrambling the genetic quadratic form Y'KY so tau[1] collapses
    // to 0. Keeping d.iid guarantees the working vectors stay aligned with K
    // (and matches R, which fits in ascending FAM-row order).
    geno_iids = d.iid;
  }
